...
document.getElementById('newContactForm').addEventListener('submit', function (event) {
  ...
  const formData = new FormData(this); // ایجاد فرم دیتا از اطلاعات فرم
  ...
  .then(data => {
      document.getElementById('responseMessage').textContent = data; // نمایش نتیجه ارسال
      document.getElementById('newContactForm').reset(); // ریست کردن فرم
  })
  .catch(error => {
      console.error('خطا در ارسال فرم:', error);
      document.getElementById('responseMessage').textContent = 'ارسال پیام با مشکل مواجه شد.';
  });
});